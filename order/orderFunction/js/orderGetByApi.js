...
module.exports.orderGetByApi = async (event) => {
    console.log(event);
    ...
    query = `Call orderGetByUser('`+userCode+`', '`+params.glnBuyer+`', '`+params.startDate+`', '`+params.endDate+`', '`+params.orderNumber+`', '`+params.formatType+`', '`+params.status+`');`;

    query = replaceAll(query, '\n        ', '');
    query = replaceAll(query, '\n    ', '');

    let resultFormats = await validateFormats(params.formatType);
    let resultStatus = await validateStatus(params.status);

    if(resultFormats == 1 && resultStatus == 1){
        return await transaction(query, params, jwt,payloadJSON);
    } else {
        return  {
            statusCode: 200,
            body: { result: false, message: 'Valor atributo formatType y/o status inválido.', records: [] },
        };
    }
...
async function transaction(query, params, jwt,payloadJSON) {
    ...
    try {
        ...
        for(var x in orders){
            ...
            switch(formatType){
                case 'JSON':
                    const orderJson = await orderJsonFormat.createFormat(order);
                    //const orderJsonToBase64 = Buffer.from(orderJson).toString('base64');
                    resultFormat.push(JSON.parse(orderJson));
                    break;
                case 'XML':
                    const orderXML = await orderXmlFormat.createFormat(order);
                    //const orderXmlToBase64 = Buffer.from(orderXML).toString('base64');
                    resultFormat.push(orderXML);
                    break;
                case 'EDI':
                    const orderEdi = await orderEancomFormat.createFormat(order);
                    //const orderEdiToBase64 = Buffer.from(orderEdi).toString('base64');
                    resultFormat.push(orderEdi);
                    break;
                default:
                    resultFormat.push({ err: 'Formato no válido.'})
                    break;
            }

            
            let queryUpdateStatus = `Call orderUpdateStatusReview(2, `+orders[x].orderCode+`);`;
            let queryResultUpdateStatus = await connection.query(queryUpdateStatus);
...
function validateFormats(formatType) {
    let result = 0;
    const allowedFormats = ['JSON', 'XML', 'EDI'];
...
function validateStatus(status) {
    let result = 0;
    const allowedStatus = ['NEW', 'VIEW_WEB', 'VIEW_API', 'ACCEPTED', 'MODIFIED', 'REJECTED'];
